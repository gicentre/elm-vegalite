...
[hTimeUnit](#hTimeUnit), [hCondition](#hCondition), [hConditions](#hConditions)
and [hStr](#hStr).
-}
type HyperlinkChannel
    ...
    | HConditions (List ( Predicate, List HyperlinkChannel )) (List HyperlinkChannel)
    | HString String
...
TODO: XXXX Deprecate empty, bind, bindScales, bindLegend, init and initInterval.
...
{-| Make a hyperlink channel conditional on a sequence of predicate values. This
can be used when several predicates need to be tested in sequence ('if-else') when
encoding a channel.
-}
hConditions : List ( Predicate, List HyperlinkChannel ) -> List HyperlinkChannel -> HyperlinkChannel
hConditions =
    HConditions
...
{-| One or more named dynamic bindings associated with a top-level parameter.
Unlike [paBind](#paBind), this allows multiple input elements to be bound to the
same field (e.g. for 'AND' type selections).
...
-- TODO Deprecate some selection properties
-- Empty
-- Bind (List Binding)
-- BindScales
-- BindLegend (List BindLegendProperty)
...
-- ][{"name":"mySelection","select": "interval",
-- "value": {"x": [{"year": 2013}, {"year": 2015}], "y": [4000, 8000]}}
--  ],
-}
...
TODO: XXXX Deprecate to reflect new interaction model.
...
hyperlinkChannelProperties field =
    case field of
        ...
        HConditions ifClauses elseClause ->
            ( "condition"
            , JE.list
                (\( predicate, ifClause ) ->
                    JE.object (predicateProperties predicate ++ List.concatMap hyperlinkChannelProperties ifClause)
                )
                ifClauses
            )
                :: List.concatMap hyperlinkChannelProperties elseClause
...
paramProperty pp =
    case pp of
        ...
        -- TODO:XXX Add the valid selection parameters
        PSelect s sps ->
            case sps of
                [] ->
