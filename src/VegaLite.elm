...
Vega-Lite runtime to generate the graphics. While this is a pure Elm package, to
...
Alternatively, to avoid coding the link to the Vega-Lite runtime, you can embed
specifications directly in a [litvis document](https://github.com/gicentre/litvis).
...
## Folding and Pivoting (data tidying)
...
[biExtent](#biExtent),[biMaxBins](#biMaxBins), [biMinStep](#biMinStep),
[biNice](#biNice), [biStep](#biStep) and [biSteps](#biSteps).
-}
type BinProperty
    = BiAnchor Float
    | Base Float
...
[cAxLabelOpacity](#cAxLabelOpacity), [cAxTickColor](#cAxTickColor), [cAxTickOpacity](#cAxOpacity),
...
[fBin](#fBin), [fSort](#fSort), [fHeader](#fHeader) and [fTimeUnit](#fTimeUnit).
-}
type FacetChannel
    = FName String
    | FmType Measurement
...
{-| Generated by functions prefixed with `le`. For example [leCornerRadius](#leCornerRadius).
-}
type LegendProperty
    = LClipHeight Float
...
[maDiscreteBandSize](#maDiscreteBandSize), [maDx](#maDx), [maDy](#maDy), [maExtent](#maExtent),
[maFill](#maFill), [maFillGradient](#maFillGradient), [maFilled](#maFilled),
...
[scClamp](#scClamp), [scInterpolate](#scInterpolate), [scNice](#scNice), [scExponent](#scExponent),
[scConstant](#scConstant), [scBase](#scBase), [scZero](#scZero) and [scReverse](#scReverse).
-}
type ScaleProperty
    = SType Scale
    | SDomain ScaleDomain
...
[seClear](#seClear), [seEncodings](#seEncodings), [seFields](#seFields), [seInit](#seInit),
[seInitInterval](#seInitInterval), [seNearest](#seNearest), [seOn](#seOn),
...
and [tFormatAsTemporal](#tFormatAsTemporal).
-}
type TextChannel
    = TName String
    | TRepeat Arrangement
...
{-| Generated by functions prefixed with `tico`.
-}
type TitleConfig
    = TAnchor Anchor
    | TAngle Float
...
[vicoDiscreteHeight](#vicoDiscreteHeight), [vicoDiscreteWidth](#vicoDiscreteWidth),
[vicoCornerRadius](#vicoCornerRadius), [vicoFill](#vicoFill), [vicoFillOpacity](#vicoFillOpacity),
[vicoOpacity](#vicoOpacity), [vicoStep](#vicoStep), [vicoStroke](#vicoStroke),
[vicoStrokeCap](#vicoStrokeCap), [vicoStrokeDash](#vicoStrokeDash),
[vicoStrokeDashOffset](#vicoStrokeDashOffset), [vicoStrokeJoin](#vicoStrokeJoin),
[vicoStrokeMiterLimit](#vicoStrokeMiterLimit), [vicoStrokeOpacity](#vicoStrokeOpacity),
and [vicoStrokeWidth](#vicoStrokeWidth).
-}
type ViewConfig
...
loess depField indField lsProps =
    (::)
        ...
        , toList
            [ JE.string depField
            , JE.string indField
            , loessPropertySpec "groupby" lsProps
            , loessPropertySpec "bandwidth" lsProps
            , loessPropertySpec "as" lsProps
            ]
        )
...
`geodata` is the primary data source containing borough boundaries and `carData`
the secondary data source containing attribute data. Both have a common data field
`boroughs` that is used to join the data sources:

    geoData =
        dataFromUrl "city.json" [ topojsonFeature "boroughs" ]

    carData =
        dataFromUrl "carOwnership.csv" []
...
fourth parameter. For a lookup that returns multiple fields, use dot notation to
...
pivot field valField pProps =
    (::)
        ...
        , toList
            [ JE.string field
            , JE.string valField
            , pivotPropertySpec "groupby" pProps
            , pivotPropertySpec "limit" pProps
            , pivotPropertySpec "op" pProps
            ]
        )
...
regression fDep fInd rgProps =
    (::)
        ...
        , toList
            [ JE.string fDep
            , JE.string fInd
            , regressionPropertySpec "groupby" rgProps
            , regressionPropertySpec "method" rgProps
            , regressionPropertySpec "order" rgProps
            , regressionPropertySpec "extent" rgProps
            , regressionPropertySpec "as" rgProps
            ]
        )
    ...
    let
        ...
        assemble ( trName, val ) =
            ...
            case trName of
                ...
                "loess" ->
                    case JD.decodeString (JD.list JD.value) (JE.encode 0 val) of
                        Ok [ dField, iField, gbObj, bwObj, asObj ] ->
                            ([ ( "loess", dField ), ( "on", iField ) ]
                                ++ (if gbObj == JE.null then
                                        []
                ...
                "pivot" ->
                    case JD.decodeString (JD.list JD.value) (JE.encode 0 val) of
                        Ok [ pField, pValField, gbObj, liObj, opObj ] ->
                            ([ ( "pivot", pField ), ( "value", pValField ) ]
                                ++ (if gbObj == JE.null then
                                        []
                                ...
                                ++ (if liObj == JE.null then
                                        []

                                    else
                                        [ ( "limit", liObj ) ]
                                   )
                                ++ (if opObj == JE.null then
                                        []
                ...
                "regression" ->
                    case JD.decodeString (JD.list JD.value) (JE.encode 0 val) of
                        Ok [ fDep, fInd, gbObj, meObj, orObj, exObj, asObj ] ->
                            ([ ( "regression", fDep ), ( "on", fInd ) ]
                                ++ (if gbObj == JE.null then
                                        []
                                ...
                                ++ (if meObj == JE.null then
                                        []

                                    else
                                        [ ( "method", meObj ) ]
                                   )
                                ++ (if orObj == JE.null then
                                        []
...
densityPropertySpec : String -> List DensityProperty -> Spec
densityPropertySpec dpName dps =
    let
        dpSpec dp =
            case dpName of
                "groupby" ->
        ...
        specList =
            List.map dpSpec dps |> List.filter (\x -> x /= JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
loessPropertySpec : String -> List LoessProperty -> Spec
loessPropertySpec lpName lps =
    let
        lpSpec lp =
            case lpName of
                "groupby" ->
        ...
        specList =
            List.map lpSpec lps |> List.filter (\x -> x /= JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
pivotPropertySpec : String -> List PivotProperty -> Spec
pivotPropertySpec ppName pps =
    let
        ppSpec pp =
            case ppName of
                "groupby" ->
        ...
        specList =
            List.map ppSpec pps |> List.filter (\x -> x /= JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
regressionPropertySpec : String -> List RegressionProperty -> Spec
regressionPropertySpec rpName rps =
    let
        rpSpec rp =
            case rpName of
                "groupby" ->
        ...
        specList =
            List.map rpSpec rps |> List.filter (\x -> x /= JE.null)
    in
    case specList of
        [ spec ] ->
            spec
