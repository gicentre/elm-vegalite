...
spaces (juxtaposition using `repeat`, `facet`, `concat`, `hConcat` or `vConcat`).
Where different views have potentially conflicting channels (e.g. two position
scales in a layered visualization) the rules for resolving them can be defined
with `resolve`. See the
...
[seEncodings](#seEncodings), [seFields](#seFields), [seInit](#seInit),
...
[`facetFlow`](#facetFlow), [`facet`](#facet), [`concat`](#concat),
...
and [repeat](#repeat). If `Nothing` or not defined, faceted small multiples will
...
{-| Initialse one or more selections with values from bound fields. For example,
