...
[dataExprs](#dataExprs) and [dataObjects](#dataObjects).
-}
type DataValues
    = Booleans (List Bool)
...
dataColumn colName data =
    case data of
        ...
        DExprs col ->
            (::) (List.map (\s -> ( colName, JE.object [ ( "expr", JE.string s ) ] )) col)
...
-- (::) (List.map (List.map (\( k, v ) -> ( k, dataValueSpec v )) >> JE.object) obs)
...
{-| A list of data values created by the given list of expressions.
-}
dataExprs : List String -> DataValues
dataExprs =
    DExprs
...
to allow all options, the mixed option types should be defined with [inDataOptions](#inDataOptions),
for example:

    ipSelect
        [ inName "Label overlap"
        , inDataOptions [ boo False, str "parity", str "greedy" ]
        ]
...
dataValuesSpecs dvs =
    case dvs of
        ...
        DExprs ss ->
            List.map (\s -> JE.object [ ( "expr", JE.string s ) ]) ss
            ...
            let
                values =
                    case vals of
                        ...
                        DExprs ss ->
                            JE.list (\s -> JE.object [ ( "expr", JE.string s ) ]) ss
