...
[`facet`](#facet), [`concat`](#concat), [`columns`](#columns), [`hConcat`](#hConcat),
[`vConcat`](#vConcat), [`spec`](#spec) and [`resolve`](#resolve).
...
and [repeat](#repeat). If not defined, faceted small multiples will be arranged
in a single row).
-}
...
{-| Specifications to be juxtaposed in a flow layout of a visualization.
-}
...
{-| Facet a view to create small multiples using a flow layout. Used when the
encoding of the visualization in small multiples is identical, but data for each
is grouped by the given fields. When creating a faceted view in this way you also
need to define a full specification to apply to each of those facets using `asSpec`.
