...
quantile field qProps =
    (::)
        ...
        , toList
            [ JE.string field
            , quantilePropertySpec "groupby" qProps
            , quantilePropertySpec "probs" qProps
            , quantilePropertySpec "step" qProps
            , quantilePropertySpec "as" qProps
            ]
        )
    ...
    let
        ...
        assemble ( trName, val ) =
            ...
            case trName of
                ...
                "quantile" ->
                    case JD.decodeString (JD.list JD.value) (JE.encode 0 val) of
                        Ok [ qField, gbObj, pObj, sObj, asObj ] ->
                            (( "quantile", qField )
                                :: (if gbObj == JE.null then
                                        []
...
densityPropertySpec dpName dps =
    let
        ...
        specList =
            List.map dpSpec dps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
loessPropertySpec lpName lps =
    let
        ...
        specList =
            List.map lpSpec lps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
pivotPropertySpec ppName pps =
    let
        ...
        specList =
            List.map ppSpec pps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
quantilePropertySpec : String -> List QuantileProperty -> Spec
quantilePropertySpec qtName qps =
    let
        qtSpec qp =
            case qtName of
                "groupby" ->
        ...
        specList =
            List.map qtSpec qps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
regressionPropertySpec rpName rps =
    let
        ...
        specList =
            List.map rpSpec rps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
stackPropertySpec sName sps =
    let
        ...
        specList =
            List.map spSpec sps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
imputePropertySpec ipName ips =
    let
        ...
        specList =
            List.map ipSpec ips |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
...
windowPropertySpec wpName wps =
    let
        ...
        specList =
            List.map wpSpec wps |> List.filter ((/=) JE.null)
    in
    case specList of
        [ spec ] ->
            spec
