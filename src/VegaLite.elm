...
type
    CInterpolate
    -- TODO: Add Expr varaiant once https://github.com/vega/vega-lite/issues/7419 is fixed.
    = CubeHelix Float
    | CubeHelixLong Float
    | Hcl
    | HclLong
...
-}
type ScaleConfig
    = SCBandPaddingInner Num
    | SCBandPaddingOuter Num
    | SCBarBandPaddingInner Num
    | SCBarBandPaddingOuter Num
    | SCRectBandPaddingInner Num
    | SCRectBandPaddingOuter Num
    | SCClamp Boo
    | SCMaxBandSize Num
    | SCMinBandSize Num
    | SCMaxFontSize Num
    ...
    | SCMaxSize Num
    | SCMinSize Num
    | SCMaxStrokeWidth Num
    | SCMinStrokeWidth Num
    | SCPointPadding Num
    | SCRound Boo
    | SCUseUnaggregatedDomain Boo
    | SCXReverse Boo
...
sacoBooExpr ex fn =
    case fn True of
        ...
        SCXReverse _ ->
            SCXReverse (BooExpr ex)

        SCRound _ ->
            SCRound (BooExpr ex)

        SCUseUnaggregatedDomain _ ->
            SCUseUnaggregatedDomain (BooExpr ex)
...
sacoNumExpr ex fn =
    case fn 0 of
        SCBandPaddingInner _ ->
            SCBandPaddingInner (NumExpr ex)

        SCBandPaddingOuter _ ->
            SCBandPaddingOuter (NumExpr ex)

        SCBarBandPaddingInner _ ->
            SCBarBandPaddingInner (NumExpr ex)

        SCBarBandPaddingOuter _ ->
            SCBarBandPaddingOuter (NumExpr ex)

        SCRectBandPaddingInner _ ->
            SCRectBandPaddingInner (NumExpr ex)

        SCRectBandPaddingOuter _ ->
            SCRectBandPaddingOuter (NumExpr ex)

        SCMaxBandSize _ ->
            SCMaxBandSize (NumExpr ex)

        SCMinBandSize _ ->
            SCMinBandSize (NumExpr ex)

        SCMaxFontSize _ ->
            SCMaxFontSize (NumExpr ex)

        SCMinFontSize _ ->
            SCMinFontSize (NumExpr ex)

        SCMaxOpacity _ ->
            SCMaxOpacity (NumExpr ex)

        SCMinOpacity _ ->
            SCMinOpacity (NumExpr ex)

        SCMaxSize _ ->
            SCMaxSize (NumExpr ex)

        SCMinSize _ ->
            SCMinSize (NumExpr ex)

        SCMaxStrokeWidth _ ->
            SCMaxStrokeWidth (NumExpr ex)

        SCMinStrokeWidth _ ->
            SCMinStrokeWidth (NumExpr ex)

        SCPointPadding _ ->
            SCPointPadding (NumExpr ex)
