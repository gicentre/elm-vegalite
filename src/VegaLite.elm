...
_interaction selections_ such as pointer-based selections or slider values. See the
Vega-Lite [parameter documentation](https://vega.github.io/vega-lite/docs/parameter.html)
for details.
...
@docs inElement
@docs inOptions
@docs inLabelledOptions
@docs inDatumOptions
@docs inLabelledDatumOptions
@docs inDataOptions
@docs inLabelledDataOptions
@docs inMin
@docs inMax
@docs inName
@docs inStep
...
[dnMinSteps](#dnMinSteps), [dnMaxSteps](#dnMaxSteps), [dnResolve](#dnResolve),
...
[inOptions](#inOptions), [inLabelledOptions](#inLabelledOptions), [inDatumOptions](#inDatumOptions),
[inLabelledDatumOptions](#inLabelledDatumOptions), [inDataOptions](#inDataOptions),
[inLabelledDataOptions](#inLabelledDataOptions), [inMin](#inMin), [inMax](#inMax), [inName](#inName),
...
type InputProperty
    ...
    | InOptions Strs
    | InLabels Strs
    | InLabelledOptions Strs Strs
    | InDatumOptions (List DataValue)
    | InLabelledDatumOptions (List DataValue) Strs
    | InDataOptions (List DataValues)
    | InLabelledDataOptions (List DataValues) Strs
    | InMin Num
    | InMax Num
    | InName Str
    | InStep Num
...
{-| Generated by [ivImputeMark](#ivImputeMark), [ivImputeDomain](#ivImputeDomain),
[ivImputePathDomain](#ivImputePathDomain) and [ivBreakPaths](ivBreakPaths).
-}
...
[maBandSize](#maBandSize), [maBaseline](#maBaseline),
[maBinSpacing](#maBinSpacing), [maBlend](#maBlend), [maBorders](#maBorders), [maBox](#maBox),
[maClip](#maClip), [maColor](#maColor), [maColorGradient](#maColorGradient),
[maCornerRadius](#maCornerRadius), [maCornerRadiusEnd](#maCornerRadiusEnd),
[maCornerRadiusTopLeft](#maCornerRadiusTopLeft), [maCornerRadiusTopRight](#maCornerRadiusTopRight),
[maCornerRadiusBottomLeft](#maCornerRadiusBottomLeft),
...
[maRadius2Offset](#maRadius2Offset), [maInvalid](#maInvalid), [maRule](#maRule), [maShape](#maShape),
...
[paSelect](#paSelect) and [paNonReact](#paNonReact).
-}
type ParamProperty
    = PBind PBinding
...
[sacoClamp](#sacoClamp), [sacoContinuousPadding](#sacoContinuousPadding), [sacoInvalid](#sacoInvalid),
[sacoMaxBandSize](#sacoMaxBandSize), [sacoMinBandSize](#sacoMinBandSize), [sacoMaxFontSize](#sacoMaxFontSize),
[sacoMinFontSize](#sacoMinFontSize), [sacoMaxOpacity](#sacoMaxOpacity), [sacoMinOpacity](#sacoMinOpacity),
[sacoMaxSize](#sacoMaxSize), [sacoMinSize](#sacoMinSize), [sacoMaxStrokeWidth](#sacoMaxStrokeWidth),
[sacoMinStrokeWidth](#sacoMinStrokeWidth), [sacoPointPadding](#sacoPointPadding), [sacoRound](#sacoRound),
...
type ScaleConfig
    ...
    | SacoInvalid (List ( Channel, DataValue ))
    | SacoMaxBandSize Num
    | SacoMinBandSize Num
    | SacoMaxFontSize Num
    | SacoMinFontSize Num
...
[doRawExpr](#doRawExpr),[doStrs](#doStrs), [doDts](#doDts), [doDtsExpr](#doDtsExpr),
[doMinDt](#doMinDt), [doMinDtExpr](#doMinDtExpr), [doMaxDt](#doMaxDt), [doMaxDtExpr](#doMaxDtExpr),
...
such as [ipColor](#ipColor). For example,
...
and [repeatFlow](#repeatFlow). For example,
...
{-| Indicates how parameters for multiple densities should be resolved. If [reIndependent](#reIndependent),
each density may have its own domain extent and dynamic number of curve sample steps. If [reShared](#reShared),
the KDE transform will ensure that all densities are defined over a shared domain and curve steps, enabling stacking.
-}
dnResolve : Resolution -> DensityProperty
dnResolve =
    DnResolve
...
{-| Specify an expression to generate an array of raw values that, if non-null, directly
overrides the domain property. Useful for interactions such as panning or zooming a scale.
The scale may be initially determined using a data-driven domain, then modified in response
to user input by setting the rawDomain value. Used by [scDomain](#scDomain).
-}
doRawExpr : String -> ScaleDomain
doRawExpr =
    DRaw
...
{-| Labelled options for an [ipRadio](#ipRadio) or [ipSelect](#ipSelect) input element.
Like [inDataOptions](#inDataOptions) except that each option has a label that can differ
from its value. Options and their corresponding labels should be paired as tuples. For example,
to label different line styles:
...
inLabelledDataOptions dPairs =
    let
        ( opts, labels ) =
            List.unzip dPairs
    in
    InLabelledDataOptions opts (Strs labels)


{-| Labelled options for an [ipRadio](#ipRadio) or [ipSelect](#ipSelect) input element.
Like [inDatumOptions](#inDatumOptions) except that each option has a label that can differ
from its value. Options and their corresponding labels should be paired as tuples. For example,
...
inLabelledDatumOptions dPairs =
    let
        ( opts, labels ) =
            List.unzip dPairs
    in
    InLabelledDatumOptions opts (Strs labels)


{-| Labelled options for an [ipRadio](#ipRadio) or [ipSelect](#ipSelect) input element.
Like [inOptions](#inOptions) but used where the text of the input label option differs
from the name of option itself. Options and their corresponding labels should be paired as tuples
with the first element being the option and the second its label. For example,
...
inLabelledOptions sPairs =
    let
        ( opts, labels ) =
            List.unzip sPairs
    in
    InLabelledOptions (Strs opts) (Strs labels)
...
{-| Determines whether or not path marks ([line](#line) and [area](#area)) should break across
invalid data. If not set, path marks will join across invalid data points. Used by [maInvalid](#maInvalid).
-}
ivBreakPaths : InvalidProperty
ivBreakPaths =
    IvBreakPaths
...
{-| Specify whether path marks ([line](#line) and [area](#area)) should impute values in their
domain. This may be set independently of whether invalid data themselves are displayed as path marks.
Used by [maInvalid](#maInvalid).
-}
ivImputePathDomain : InvalidProperty
ivImputePathDomain =
    IvImputePathDomain
...
selection to link with the primary data field. This is similar to [lookup](#lookup)
      ...
      ps =
          params
              << param "mySel"
                  [ paSelect sePoint [ seOn "pointerover", seEncodings [ chX ] ] ]
...
{-| Specify how or whether invalid data items (`null` values, `NaNs` etc.) are represented as marks.
This provides a slightly more intuitive way of naming the rules that determine mark and scale encoding
of invalid data. Here's a comparison with the [Vega-Lite invalid mode options](https://vega.github.io/vega-lite/docs/invalid-data.html#mark-invalid-mode):

[filter](https://vega.github.io/vega-lite/docs/invalid-data.html#mark-invalid-mode) ➛ `maInvalid []` (ignore all invalid data when encoding marks and domains)

[show](https://vega.github.io/vega-lite/docs/invalid-data.html#mark-invalid-mode) ➛ `maInvalid [ ivImputeMark ]` (impute all invalid data when encoding marks and domains)

[break-paths-filter-domains](https://vega.github.io/vega-lite/docs/invalid-data.html#mark-invalid-mode) ➛ `maInvalid [ ivBreakPaths ]` (like filter but break path marks around invalid data)

[break-paths-show-domains](https://vega.github.io/vega-lite/docs/invalid-data.html#mark-invalid-mode) ➛ `maInvalid [ ivImputeDomain, ivImputePathDomain, ivBreakPaths ]` (like break-paths-filter-domains but use imputed values when calculating domains)

[break-paths-show-path-domains](https://vega.github.io/vega-lite/docs/invalid-data.html#mark-invalid-mode) ➛ `maInvalid [ ivImputePathDomain, ivBreakPaths ]` (like break-paths-show-domains but only use imputed values when calculating domains of path marks)

-}
maInvalid : List InvalidProperty -> MarkProperty
maInvalid =
    MInvalid
...
{-| Deprecated. Use [maInvalid](#maInvalid) instead.
-}
...
Used by [padding](#padding) and [coPadding](#coPadding). For example,
...
param nme pps =
    (::) ( nme, List.concatMap paramProperty pps |> JE.object )
...
{-| Specify a new non-reactive paramter based on the given parameter name. This can be useful
when an interactive legend is used to filter results, but you wish the initial legend encoding
to remain.

In this example, `legSel` is an interactive color legend; `clr` extracts the domain
of the colours before any interaction takes place and `clrDomain` captures that initial domain.
With interaction `clr` will get updated, but the non-reactive `clrDomain` will not and can
therefore be used to set a fixed color scale domain:
...
{-| When specifying a [resolution](#resolution), indicates that a channel should be independent
of others in a composite visualization. When specifying [dnResolve](#dnResolve), indicates density
paramters should not be shared between groups.
-}
...
shared with others in a composite visualization. When specifying [dnResolve](#dnResolve),
indicates settings in a density transform should be shared between groups.
-}
reShared : Resolution
reShared =
    RShared
...
{-| The imputed value to use when enocding invalid data (e.g. nulls or NaNs) on a per-channel basis.
Used by [coScale](#coScale). For example, the following will ensure all invalid data are shown as red
when encoded in the color channel and size 4 when encoded with the size channel,

    cfg =
        configure
            << configuration (coMark [ maInvalid [ ivImputeMark ] ])
            << configuration
                (coScale
                    [ sacoInvalid
                        [ ( ChColor, str "red" )
...
-}
sacoInvalid : List ( Channel, DataValue ) -> ScaleConfig
sacoInvalid =
    SacoInvalid
...
{-| A band scale. Used when specifying a [scType](#scType).
-}
scBand : Scale
scBand =
    ScBand
...
{-| An ordinal band scale. Used when specifying a [scType](#scType).
-}
scBinOrdinal : Scale
scBinOrdinal =
    ScBinOrdinal
...
{-| A linear scale. Used when specifying a [scType](#scType).
-}
scLinear : Scale
scLinear =
    ScLinear
...
{-| A log scale used when specifying a [scType](#scType). Defaults to log of base 10,
...
{-| An ordinal scale. Used when specifying a [scType](#scType).
-}
scOrdinal : Scale
scOrdinal =
    ScOrdinal
...
{-| A point scale. Used when specifying a [scType](#scType).
-}
scPoint : Scale
scPoint =
    ScPoint
...
{-| A power scale used when specifying a [scType](#scType). The exponent to use
...
{-| A quantile scale. Used when specifying a [scType](#scType).
-}
scQuantile : Scale
scQuantile =
    ScQuantile
...
{-| A quantizing scale. Used when specifying a [scType](#scType).
-}
scQuantize : Scale
scQuantize =
    ScQuantize
...
{-| A square root scale. Used when specifying a [scType](#scType).
-}
scSqrt : Scale
scSqrt =
    ScSqrt
...
{-| A threshold scale. Used when specifying a [scType](#scType).
-}
scThreshold : Scale
scThreshold =
    ScThreshold
...
{-| A temporal scale. Used when specifying a [scType](#scType).
-}
scTime : Scale
scTime =
    ScTime
...
{-| A UTC temporal scale. Used when specifying a [scType](#scType).
-}
scUtc : Scale
scUtc =
    ScUtc
...
paintbrush effect under the pointer:
    ...
    ps =
        params
            << param "paint" [ paSelect sePoint [ seOn "pointerover" ] ]
...
under the pointer while the shift key is pressed down:
    ...
    ps =
        params
            << param "paint"
                [ paSelect sePoint [ seOn "pointerover", seToggle tpShiftKey ] ]
...
densityProperty denProp =
    case denProp of
        ...
        DnResolve r ->
            [ ( "resolve", resolutionLabel r |> JE.string ) ]
...
inputProperty prop =
    case prop of
        ...
        InLabelledOptions opts labels ->
            List.concat [ inputProperty (InOptions opts), inputProperty (InLabels labels) ]

        InLabelledDataOptions opts labels ->
            List.concat [ inputProperty (InDataOptions opts), inputProperty (InLabels labels) ]

        InLabelledDatumOptions opts labels ->
            List.concat [ inputProperty (InDatumOptions opts), inputProperty (InLabels labels) ]
...
markProperty mProp =
    case mProp of
        ...
        MInvalid ivs ->
            if List.member IvImputeMark ivs then
                [ ( "invalid", JE.string "show" ) ]

            else if ivs == [ IvBreakPaths ] then
                [ ( "invalid", JE.string "break-paths-filter-domains" ) ]

            else if ivs == [] then
                [ ( "invalid", JE.string "filter" ) ]

            else if
                List.member IvImputeDomain ivs
                    && List.member IvImputePathDomain ivs
                    && List.member IvBreakPaths ivs
            then
                [ ( "invalid", JE.string "break-paths-show-domains" ) ]

            else
                [ ( "invalid", JE.string "break-paths-show-path-domains" ) ]
...
paramProperty : ParamProperty -> List LabelledSpec
paramProperty pp =
    case pp of
        PBind bps ->
        ...
        PNonReact s ->
            [ ( "expr", JE.string s ), ( "react", JE.bool False ) ]
...
scaleConfigProperty scaleCfg =
    case scaleCfg of
        ...
        SacoInvalid imps ->
            [ ( "invalid", JE.object (List.map (\( ch, dv ) -> ( channelLabel ch, JE.object [ ( "value", dataValueSpec dv ) ] )) imps) ) ]
