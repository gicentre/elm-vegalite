...
type ConfigurationProperty
    ...
    | AxisDiscrete (List AxisConfig)
    | AxisPoint (List AxisConfig)
    | AxisQuant (List AxisConfig)
    | AxisTemporal (List AxisConfig)
    | Background String
...
{-| Indicate configuration should be applied to both axes.
-}
axBoth : AxisChoice
axBoth =
    AxBoth
...
{- | Set the opacity of unselected legend items when made interactive via
   [seBindLegend](#seBindLegend).
   -
   -- TODO: Pending resoltion of https://github.com/vega/vega-lite/issues/5921
...
configProperty configProp =
    case configProp of
        ...
        AxisDiscrete acs ->
            ( "axisDiscrete", JE.object (List.map axisConfigProperty acs) )

        AxisPoint acs ->
            ( "axisPoint", JE.object (List.map axisConfigProperty acs) )

        AxisQuant acs ->
            ( "axisQuantitative", JE.object (List.map axisConfigProperty acs) )

        AxisTemporal acs ->
            ( "axisTemporal", JE.object (List.map axisConfigProperty acs) )
