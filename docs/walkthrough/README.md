...
This walkthrough will introduce you to the principles and coding style for using elm-vegaLite to create interactive visualizations in the [Elm](http://elm-lang.org) language. It is based on the talk given by [Wongsuphasawat et al at the 2017 Open Vis Conf](https://youtu.be/9uaHRWj04D4). If you wish to follow along with their talk, timings are given by each section.
...
let
    ...
    spec =
        asSpec
            [ dataFromUrl (path ++ "seattle-weather.csv") [], enc [], bar [] ]
in
toVegaLite
...
let
    ...
    spec =
        asSpec [ dataFromUrl (path ++ "seattle-weather.csv") [], enc [], point [] ]
in
toVegaLite
    [ repeat
...
scatterProps =
    ...
    in
    [ dataFromUrl (path ++ "cars.json") [], trans [], enc [], circle [] ]
```
...
Notice here that to project the selection we parameterise the `select` not with a field name as we have done previously but with a channel encoding using the function `seEncodings` (here parameterised with the X-position channel function `chX`).
